...
SELECT *
FROM customers
WHERE UPPER(postal_code) in ('1010')
```
...
SELECT *
FROM customers
WHERE UPPER(company_name) LIKE '%MARKET%'
```
...
```SQL
SELECT c.company_name, COUNT(o.order_id) orderCount
...
```SQL
SELECT c.contact_name, COUNT(o.order_id) orderCount
FROM orders o LEFT JOIN customers c 
ON c.customer_id = o.customer_id
